...
module.exports = class NcdSerialProXR{
	...
	timerOp(op, r, d){
		var obj = this;
		//return this.send([254,50,131,255,255]);//.then().catch(console.log);
		return new Promise((fulfill, reject) => {
	...
	relayTalk(str){
		var groups = ['relay', 'group', 'bank', 'banks', 'op', 'mtype', 'duration', 'scale'];
		var m;
		var regex = /(?:(?:relay|channel) ([0-9]+))|(all|odd|even)|bank ([0-9]+)|(all) banks|(off|activate|deactivate|toggle|on)|(for|in) ([0-9]+) ([^\s]+)/gmi;
		var res = {};
	...
	status(){
		if(typeof this.interactive.relay == 'undefined'){
			var bank = typeof this.interactive.bank == 'undefined' ? 1 : this.interactive.bank;
			this.interactive = {};
			return this.bankStatus(bank);
		}
		var ret = this.relayStatus(this.interactive.relay, this.interactive.bank);
		this.interactive = {};
		return ret;
	}
	readAD(b){
